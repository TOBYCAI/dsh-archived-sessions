...
- **会话详情（v2.0.0）**：每条会话可点「详情」展开，展示**磁盘占用**、**轮次 / 步骤 / 用户·助手消息 / 工具调用 / 图片附件**统计、**工具使用分布**、**搜索·抓取记录**、**write/edit 写过的文件列表**（已过滤磁盘上已不存在的路径）、以及**血统**（父会话 / 子会话 / 子代理），便于了解每条会话的成本与产出、清理大文件。灵感参考自社区插件 `Zephyr-vibe/dsh-archived-sessions` 的 buildDetails（取其精华）。
